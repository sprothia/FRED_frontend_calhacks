...
//
    ...
    @IBAction func getHelp(_ sender: Any) {
        ...
        let voiceText = "dummy voice text"
        var roundedLat = 0.0
        var roundedLong = 0.0
        ...
        {
            roundedLat = Double(round(1000 * currentLocation.coordinate.latitude.magnitude) / 1000)
            roundedLong = Double(round(1000 * currentLocation.coordinate.longitude.magnitude) / 1000)
        }
        
        showAlertDone(message: "Help is on the way.")
        
        var transcribedAudioString = ""
        if let url = audioFileName {
            transcribeAudio(url: url) { transcription in
                DispatchQueue.main.async {
                    transcribedAudioString = transcription
                }
            }
        }
        ...
        let imagesData: [String: Any] = [
            ...
            "voiceText": transcribedAudioString,
            "lat": String(roundedLat),
            "long": String(roundedLong)
        ]
